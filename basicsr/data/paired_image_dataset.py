...
# sam = sam_model_registry["vit_h"](checkpoint="/data_8T1/wangcong/CVPR24/models_sam/sam_vit_h_4b8939.pth")
# sam = sam_model_registry["vit_b"](checkpoint="/data_8T1/wangcong/CVPR24/models_sam/sam_vit_b_01ec64.pth")
...
class PairedImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print("img_lq_mask", img_lq_mask)
        # print("img_lq_mask*255", img_lq_mask*255)
        # print('path',gt_path,lq_path,lq_mask_path)
        # print('img_lq_mask',img_lq_mask)
        # print('img_lq', img_lq)
        ...
        if self.opt['phase'] == 'train':
            ...
            # padding
            h, w, c = img_lq_mask.shape
            # img_gt, img_lq, img_lq_mask = padding(img_gt, img_lq, img_lq_mask, gt_size)
            if h < gt_size or w < gt_size:
                img_gt, img_lq, img_lq_mask = cv2.resize(img_gt, (gt_size, gt_size)), cv2.resize(img_lq, (gt_size, gt_size)), cv2.resize(img_lq_mask, (gt_size,gt_size))
            # print('img_gt', img_gt.shape, img_lq.shape, img_lq_mask.shape)
            # print('img_lq', img_lq.shape)
            # print('img_lq_mask', img_lq_mask.shape)
            # random crop
            img_gt, img_lq, img_lq_mask = paired_random_crop(img_gt, img_lq, img_lq_mask, gt_size, scale,
                                                gt_path)

            # flip, rotation augmentations
            if self.geometric_augs:
                img_gt, img_lq, img_lq_mask = random_augmentation(img_gt, img_lq, img_lq_mask)
        # print('img_lq_mask',img_lq_mask)
        ...
        if self.mean is not None or self.std is not None:
            ...
            normalize(img_lq_mask, self.mean, self.std, inplace=True)
            normalize(img_gt, self.mean, self.std, inplace=True)

            # print('normalize-normalize-normalize-normalize-normalize-normalize-normalize-normalize')
        # print('img_gt---------', img_gt.shape, img_lq.shape, img_lq_mask.shape)
        # print('ormalize(img_lq_mask, self.mean, self.std, inplace=True)', img_lq_mask)
        return {
            'lq': img_lq,
            'lq_mask': img_lq_mask,
            'gt': img_gt,
